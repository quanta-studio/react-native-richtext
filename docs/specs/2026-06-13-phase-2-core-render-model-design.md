...
Depends on: Phase 0 (`@quanta-studio/react-native-richtext-dom`) and Phase 1 (`@quanta-studio/react-native-richtext-css`),
...
- Name: `@quanta-studio/react-native-richtext-core`. React-free. Depends on the `@quanta-studio/react-native-richtext-dom` and
  `@quanta-studio/react-native-richtext-css` workspace packages (the latter for the `ComputedStyle`/`RNStyle`
...
A tested, React-free `@quanta-studio/react-native-richtext-core` package exposing `buildRenderTree(document,
