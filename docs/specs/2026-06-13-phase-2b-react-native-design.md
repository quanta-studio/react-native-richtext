...
Depends on: Phase 0 (`@quanta-studio/react-native-richtext-dom`), Phase 1 (`@quanta-studio/react-native-richtext-css`),
Phase 2a (`@quanta-studio/react-native-richtext-core`) — all merged. Read the architecture doc
...
Sub-project 2b: **`@quanta-studio/react-native-richtext`**, the flagship public package. It is the first
...
| Question               | Decision                                                                                                                                                                                                                                                                                    |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cycle scope            | **Package + minimal example app.** Build `@quanta-studio/react-native-richtext` (tested) plus a minimal Expo screen for visual validation. The cross-repo dogfood (`OutletAboutScreen`) and the canary publish are deferred to a follow-on (they need the fonerewards repo + an NPM token). |
...
- Name: **`@quanta-studio/react-native-richtext`** (the headline public package), directory `packages/react-native`.
  This is the first React/RN package — **not** React-free.
- Runtime dependencies: the three workspace packages `@quanta-studio/react-native-richtext-dom`,
  `@quanta-studio/react-native-richtext-css`, `@quanta-studio/react-native-richtext-core`.
...
`RNStyle` is re-exported from `@quanta-studio/react-native-richtext-css` (via core). `useRichTextContext()` returns
...
A tested `@quanta-studio/react-native-richtext` package exposing `<RichText>` (orchestration + registry + default
