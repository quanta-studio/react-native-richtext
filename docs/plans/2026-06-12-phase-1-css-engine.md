...
**Goal:** Build `@quanta-studio/react-native-richtext-css` — a React-free package whose `resolveStyles(document, options)` turns a parsed DOM plus consumer styles into a fully-computed `Map<Element, ComputedStyle>` (cascade + inheritance applied, relative units resolved), with optional diagnostics.
...
Typecheck the package with: `pnpm --filter @quanta-studio/react-native-richtext-css typecheck`

---

## Task 0: Scaffold the `@quanta-studio/react-native-richtext-css` package
...
```json
{
  "name": "@quanta-studio/react-native-richtext-css",
  "version": "0.0.0",
  ...
  "dependencies": {
    "@quanta-studio/react-native-richtext-dom": "workspace:*",
    "css-select": "^6.0.0",
...
Run: `pnpm --filter @quanta-studio/react-native-richtext-css typecheck && pnpm --filter @quanta-studio/react-native-richtext-css build`
...
git commit -m "chore(css): scaffold @quanta-studio/react-native-richtext-css package"
```
...
```ts
import { getElementsByTagName, getText, isTag } from '@quanta-studio/react-native-richtext-dom'
...
import { getElementsByTagName, getAttributeValue } from '@quanta-studio/react-native-richtext-dom'
import type { Document, Element } from '@quanta-studio/react-native-richtext-dom'
import type { Rule } from '../types'
...
Run: `pnpm --filter @quanta-studio/react-native-richtext-css typecheck`
Expected: exit 0.
