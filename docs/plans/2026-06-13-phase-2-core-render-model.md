...
**Goal:** Build `@yk-yong/rn-rich-text-core` — a React-free package whose `buildRenderTree(document, styles)` turns a parsed DOM plus the css `Map<Element, ComputedStyle>` into a renderer-agnostic styled tree (block/inline split, whitespace-collapsed + entity-decoded text, basic list markers).
...
**Tech Stack:** TypeScript 6 (strict, `verbatimModuleSyntax`, `noUncheckedIndexedAccess`), Vitest 4, tsup, pnpm. New dep: `entities`. Workspace deps: `@yk-yong/rn-rich-text-dom`, `@yk-yong/rn-rich-text-css`.
...
Typecheck the package: `pnpm --filter @yk-yong/rn-rich-text-core typecheck`
...
```json
{
  "name": "@yk-yong/rn-rich-text-core",
  "version": "0.0.0",
  "description": "Render-model builder for rn-rich-text: DOM + computed styles -> renderer-agnostic styled tree. React-free.",
  "license": "MIT",
  "type": "module",
  "sideEffects": false,
  "main": "./dist/index.cjs",
...
Render-model builder for rn-rich-text: turns a parsed DOM + per-element computed
...
```bash
pnpm --filter @yk-yong/rn-rich-text-core add @yk-yong/rn-rich-text-dom@workspace:* @yk-yong/rn-rich-text-css@workspace:* entities
pnpm --filter @yk-yong/rn-rich-text-core add -D @types/node
```
...
Run: `pnpm install && pnpm --filter @yk-yong/rn-rich-text-core typecheck && pnpm --filter @yk-yong/rn-rich-text-core build`
...
git commit -m "chore(core): scaffold @yk-yong/rn-rich-text-core package"
```
...
- [ ] **Step 4: Run → PASS**, then typecheck (`pnpm --filter @yk-yong/rn-rich-text-core typecheck`) → clean.
...
- [ ] **Step 5: Run → PASS**; whole-package typecheck (`pnpm --filter @yk-yong/rn-rich-text-core typecheck`) → clean.
