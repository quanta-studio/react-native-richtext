...
Phase 0's deliverable (a tested, React-free `@quanta-studio/react-native-richtext-dom` package + green CI) is
...
`@quanta-studio/react-native-richtext-dom`. Consider adding an `engines` field to the package itself.
